...
// In-memory student array (full objects with attendance)
let students = [];
    ...
    if (stored) {
        try {
            ...
            if (Array.isArray(parsed) && parsed.length > 0) {
                students = parsed;
                // Ensure each student has present/attendanceTime fields
                students.forEach(s => {
...
// ----- Save students list (without attendance) -----
...
// ----- Load attendance from localStorage and apply to students -----
...
// ----- Save both students and attendance -----
...
let currentMode = 'admin'; // 'admin' | 'attendance'
...
function switchMode(mode) {
    ...
    // Re-render table with appropriate actions
    renderStudentTable();
...
function renderStudentTable() {
    ...
    const containerId = isAdmin ? 'studentTableContainerAdmin' : 'studentTableContainer';
    const filterId = isAdmin ? 'tableFilterAdmin' : 'tableFilter';
...
function displayStudentAttendance(student) {
    ...
    if (student.present) {
        ...
        msg.textContent = 'Attendance already marked.';
...
function confirmMarkAttendance() {
    ...
    if (currentSearchStudent.present) {
        document.getElementById('attendanceMessage').textContent = 'Attendance already marked.';
...
function removeStudent(rollNo) {
    ...
    updateAllStats();
    renderStudentTable();
    // Clear admin card if this student was shown
...
function saveEditStudent() {
    ...
    // If roll number changed, check uniqueness
    ...
    // If this student was shown in admin card, refresh it
    if (currentAdminSearchStudent && currentAdminSearchStudent.rollNo === rollNo) {
        displayStudentAdmin(student);
    }
}
...
function correctAttendance(rollNo) {
    ...
    // Update admin card if this student is shown
    if (currentAdminSearchStudent && currentAdminSearchStudent.rollNo === rollNo) {
        displayStudentAdmin(student);
    }
    // Update attendance mode card if this student is shown
...
function init() {
    ...
    // Table filters
    document.getElementById('tableFilter').addEventListener('input', renderStudentTable);
    ...
    // Clear file inputs after import (optional)
    ...
    console.log('✅ Event Attendance System initialized.');
