...
// All existing Admin Mode functionality remains unchanged.
...
function confirmMarkAttendance() {
    ...
    // Auto-prepare for next student after a short delay
    autoClearTimeout = setTimeout(function() {
...
function prepareForNextStudent() {
    ...
    // Reset state
    currentSearchStudent = null;
    // Focus on input for next student
    document.getElementById('rollInput').focus();
}
...
document.getElementById('rollInput').addEventListener('keydown', function(e) {
    if (e.key === 'Enter') {
        e.preventDefault();
        // If student card is shown and confirm button is enabled, pressing Enter should confirm
        const card = document.getElementById('studentCard');
        const confirmBtn = document.getElementById('confirmMarkBtn');
...
function init() {
    ...
    console.log('✅ Event Attendance System initialized (Phase 6A - Fast Check-in).');
}
