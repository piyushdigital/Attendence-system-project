...
// We no longer need autoClearTimeout
...
function switchMode(mode) {
    ...
    if (mode === 'admin') {
        ...
        // Focus the roll input when switching to Attendance Mode
...
// ============================================================
// 5. ATTENDANCE MODE — SEARCH & VERIFICATION
// ============================================================
...
function performSearchAttendance() {
    ...
    // Focus stays on input for next action
}
...
// ============================================================
// 6. ATTENDANCE MODE — CONFIRM & CANCEL
// ============================================================
...
function confirmMarkAttendance() {
    ...
    // Clear the input and focus it for the next student
    document.getElementById('rollInput').value = '';
    document.getElementById('rollInput').focus();

    // Do NOT hide the card; keep the success state visible.
}
...
// Enter key on search input triggers search or confirm if applicable
document.getElementById('rollInput').addEventListener('keydown', function(e) {
    if (e.key === 'Enter') {
        e.preventDefault();
        // If student card is shown and confirm button is enabled, pressing Enter confirms
        const card = document.getElementById('studentCard');
        const confirmBtn = document.getElementById('confirmMarkBtn');
        if (!card.classList.contains('hidden') && !confirmBtn.disabled) {
            // Student is displayed and can be confirmed
...
function correctAttendance(rollNo) {
    ...
    // If this student is currently shown in Attendance Mode card, update it
...
function init() {
    ...
    // Enter key is handled by the keydown listener on the input
    ...
    console.log('✅ Event Attendance System initialized (Phase 6A - Fast Check-in, success stays visible).');
}
